...
import { handlePhilosopherQuotes, handleAnimeQuotes, handleQrCode, handleBarcode, handleUsernameGen, handleJokes, handleCurrencyConvert, handleAsciiArt, handleRandomName, handleGradientBg, handleTextStats, handleIpInfo, handleColorInfo, handleShortUrl, handleRandomFacts } from './endpoints.js';
...
export async function handleDashPlugins(req,res) {
  if(req.method!=='GET') return fail(res,'Method not allowed',405);
  if(!isConfigured()) return fail(res,'GitHub belum dikonfigurasi. Set GITHUB_TOKEN & GITHUB_REPO di Vercel env.',503);
  try{
    const url=`${ghBase()}/contents/${ghPluginsPath()}?ref=${ghBranch()}`;
    const r=await fetch(url,{headers:ghHeaders(),signal:AbortSignal.timeout(8000)});
    if(r.status===404) return fail(res,'Folder plugin tidak ditemukan di repo. Buat folder "plugins" dulu di repo.',404);
    if(!r.ok) {
      const errText=await r.text();
      return fail(res,`GitHub API error ${r.status}: ${errText.slice(0,100)}`,502);
    }
    const files=await r.json();
    if(!Array.isArray(files)) return fail(res,'GitHub response bukan array. Check folder path.',502);
